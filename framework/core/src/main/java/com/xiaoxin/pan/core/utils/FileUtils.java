...
public class FileUtils {
    ...
    public static String getFileSuffix(String filename) {
        if (StringUtils.isBlank(filename) || filename.lastIndexOf(XPanConstants.POINT_STR) == XPanConstants.MINUS_ONE_INT) {
            return XPanConstants.EMPTY_STR;
        }
        return filename.substring(filename.lastIndexOf(XPanConstants.POINT_STR)).toLowerCase();
    }
    ...
    public static String getFileExtName(String filename) {
        ...
        return filename.substring(filename.lastIndexOf(XPanConstants.POINT_STR) + XPanConstants.ONE_INT).toLowerCase();
    }
    ...
    public static String generateStoreFileChunkRealPath(String basePath, String identifier, Integer chunkNumber) {
        return new StringBuffer(basePath)
                ...
                .append(File.separator)
                .append(UUIDUtil.getUUID())
                .append(XPanConstants.COMMON_SEPARATOR)
                .append(chunkNumber)
                .toString();
    }
...
//        writeStream2StreamRange(fileInputStream, outputStream, XPanConstants.ZERO_LONG, length);
    ...
    public static void writeStream2StreamNormal(InputStream inputStream, OutputStream outputStream) throws IOException {
        ...
        int len;
        while ((len = inputStream.read(buffer)) != XPanConstants.MINUS_ONE_INT) {
            outputStream.write(buffer, XPanConstants.ZERO_INT, len);
        }
        outputStream.flush();
        inputStream.close();
        outputStream.close();
