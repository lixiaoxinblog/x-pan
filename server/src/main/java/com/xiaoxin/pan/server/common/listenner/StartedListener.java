package com.xiaoxin.pan.server.common.listenner;
...
public class StartedListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent applicationReadyEvent) {
        ...
        log.info(AnsiOutput.toString(AnsiColor.BRIGHT_BLUE, "x pan server started at: ", serverUrl));
        if (checkShowServerDoc(applicationReadyEvent.getApplicationContext())) {
            log.info(AnsiOutput.toString(AnsiColor.BRIGHT_BLUE, "x pan server's doc started at:", serverUrl + "/doc.html"));
        }
        log.info(AnsiOutput.toString(AnsiColor.BRIGHT_YELLOW, "x pan server has started successfully!"));
    }
